...
def calib_fisheye(filename, checkerboard=(5, 5)):
    ...
    if vid_length > 1000:
        pictures = [
            np.asarray(video.get_data(i)) for i in range(vid_length) if i % 5 == 0
        ]
    else:
...
def calib_camera_charuco(filename, s):
    ...
    board = aruco.CharucoBoard_create(
        squaresX=5, squaresY=7, squareLength=10, markerLength=8, dictionary=ARUCO_DICT
    )

    if vid_length > 1000:
        pictures = [
            np.asarray(video.get_data(i)) for i in range(vid_length) if i % s == 0
        ]
    else:
        pictures = [np.asarray(video.get_data(i)) for i in range(vid_length)]
    ...
    ret, K, D, rvec, tvec = cv2.aruco.calibrateCameraCharuco(
